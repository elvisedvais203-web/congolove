...
import { commentFeedPost, createFeedPost, followUser, getMyNetwork, getSuggestions, listFeedPosts, listSavedFeedPosts, toggleFeedLike, toggleFeedSave, unfollowUser } from "../services/nextalksocial.service";
...
export async function toggleSave(req: AuthRequest, res: Response) {
  const userId = req.user!.userId;
  const postId = String(req.params.postId);
  const data = await toggleFeedSave(userId, postId);
  res.json(data);
}

export async function savedFeed(req: AuthRequest, res: Response) {
  const userId = req.user!.userId;
  const limit = Number(req.query.limit ?? 30);
  const data = await listSavedFeedPosts(userId, limit);
  res.json(data);
}
