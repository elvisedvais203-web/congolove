...
export async function listFeedPosts(userId: string, limit: number) {
  ...
  return posts.map((post) => ({
    ...
    savedByMe: post.saves.some((item) => item.userId === userId),
...
export async function toggleFeedSave(userId: string, postId: string) {
  ...
  await prisma.feedPostSave.create({ data: { postId, userId } });
  return { saved: true };
}

export async function listSavedFeedPosts(userId: string, limit: number) {
  const rows = await prisma.feedPostSave.findMany({
    where: { userId },
    include: {
      post: {
        include: {
